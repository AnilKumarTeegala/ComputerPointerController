...
class GazeEstimationModel:
    ...
    def preprocess_output(self, outputs,hpa):
        ...
        rollValue = hpa[2]
        cosValue = np.cos((rollValue * np.pi) / 180.0)
        sinValue = np.sin((rollValue * np.pi) / 180.0)
        
        new_x = gaze_vector[0] * cosValue + gaze_vector[1] * sinValue
        new_y = -gaze_vector[0] *  sinValue+ gaze_vector[1] * cosValue
